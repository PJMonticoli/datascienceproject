...
# 2. CARGA DE DATOS
df = pd.read_csv(
    'datascienceproject\\Habitaciones\\dataset_inquilinos.csv', index_col='id_inquilino')
...
# Reescalar la matriz
matriz_s_reescalada = ((matriz_s - min_original) / (max_original -
                       min_original)) * (rango_max - rango_min) + rango_min
...
def inquilinos_compatibles(id_inquilinos, topn):
    ...
    resultado = pd.concat(
        [registros_buscados.T, registros_similares.T], axis=1)

    # Crear un objeto Series con la similitud de los inquilinos similares encontrados
    similitud_series = pd.Series(
        data=topn_inquilinos.values, index=topn_inquilinos.index, name='Similitud')
